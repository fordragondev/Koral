...
> **Verification note:** This environment blocked WebSearch, Brave, and Context7 MCP access during research, so version numbers below reflect the last confirmed state (training cutoff May 2025) projected forward to April 2026. Before locking `package.json`, run `npm view <pkg> version` for each core dependency and skim the changelog for the major version in use. Rationale and architectural choices are unaffected by minor version drift.

## Executive Recommendation

**Ship this stack:** Next.js 15 (App Router, RSC) + TypeScript 5.6+ + Drizzle ORM + Neon Postgres + Auth.js v5 (Credentials + OAuth, per-tenant isolation via tenant-scoped user tables) + Pusher Channels for realtime + Cloudflare R2 for media + `next/image` + Postgres FTS (Meilisearch later) + `next-intl` + Tiptap + Vercel middleware for tenant routing + Inngest for background jobs + Axiom (or Vercel Observability + Sentry) for logs/errors.

This is optimized for: **solo-dev velocity, SEO, low ops, Vercel-serverless compatibility, and the per-domain account silo requirement.**
...
Start with **Postgres full-text search** (`tsvector` columns + GIN indexes, `websearch_to_tsquery`, `ts_rank_cd`). It's free, already in your DB, and sufficient up to tens of thousands of articles/posts per tenant. Model:
...
**Graduate to Meilisearch** (self-hosted on Fly.io or Meilisearch Cloud) when: you need typo tolerance, faceted filtering, instant-search UX, or Postgres FTS tuning is costing you time. Meilisearch beats Typesense on DX and beats Algolia on price. **Do NOT start with Algolia** — pricing scales brutally for UGC. **Do NOT start with Elasticsearch** — ops burden.
...
pnpm add inngest @upstash/redis @upstash/ratelimit @vercel/flags

# Observability
pnpm add @sentry/nextjs next-axiom posthog-js posthog-node
...
# Utilities
pnpm add date-fns date-fns-tz thumbhash

# Dev tooling
pnpm add -D vitest @vitejs/plugin-react @testing-library/react @testing-library/jest-dom jsdom
pnpm add -D @playwright/test
pnpm add -D @biomejs/biome   # OR: eslint prettier eslint-config-next
```

## Multi-Tenant Routing Pattern (Vercel + Next.js 15)

This is the load-bearing pattern for the whole project.

**Domain → Tenant resolution** happens in `middleware.ts` (Edge runtime):
...
export async function middleware(req: NextRequest) {
  ...
  const tenant = resolveTenantFromHost(host); // aquariumcommu.com → 'aquarium'
  if (!tenant) return new NextResponse('Unknown tenant', { status: 404 });

  // Stash tenant for server components / route handlers via header
  const res = intl(req);
  res.headers.set('x-tenant', tenant.slug);
...
**If photo traffic explodes (>1TB egress/month):**
...
**If realtime costs blow up on Pusher:**
...
**If you need deeper SSO / enterprise auth (not v1):**
...
**If AI-drafted content workflow needs orchestration:**
...
> **Action item before code:** Run `npm view <pkg> version` on: `next`, `react`, `drizzle-orm`, `next-auth`, `next-intl`, `@tiptap/react`, `tailwindcss`, `inngest`, `@sentry/nextjs`, `pusher`. Lock majors in `package.json`. Skim the changelog for each major. Training-data versions above may be one minor behind reality as of April 2026.

---
*Stack research for: Multi-tenant niche social platform (Koral / aquariumcommu.com)*
*Researched: 2026-04-13*
